...
def generate(database_name):
    ...
    for table in tables:
        ...
        # p = markdown_table_header % ('', remove_newline(table[1]))
        # p = markdown_table_header % (remove_newline(table[1]),'')
        num = num +1
        p = markdown_table_header % (num,remove_newline(table[1]), table[0])
...
if __name__ == '__main__':
    mysqlDB_name = 'cloud_course'
    generate(mysqlDB_name)